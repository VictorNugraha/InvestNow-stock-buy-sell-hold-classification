...
```{r}
isat_analisa2$final_decision[isat_analisa2$date == "2019-12-06"] <- "Sell"
```
...
init <- initial_split(data = isat_cv,
                      prop = 0.65, 
                      strata = final_decision) 

isat_train <- training(init)
isat_test <- testing(init)
```

```{r}
prop.table(table(isat_train$final_decision))
...
```{r}
set.seed(123)
isat_train_smote <- SmoteClassif(final_decision ~ ., dat, C.perc = list(Buy = 25, Hold = 1, Sell = 22), dist = "HVDM")

prop.table(table(isat_train_smote$final_decision))
...
```{r, fig.dim = c(50 , 50)}
model_dt <- ctree(final_decision ~ ., data = isat_train_smote)

saveRDS(model_dt, "isat_dt.RDS")

plot(model_dt, type = "simple")
```

```{r}
pred_model_dt <- predict(object = model_dt, newdata = isat_test, type = "response")
...
isat_forest <- readRDS("isat_forest.RDS")
isat_forest
```

```{r}
rm_pred <- predict(isat_forest, isat_test, type = "raw")
head(rm_pred)
```
