...
```{r}
library(data.table)
bbri_backtest$total_stock <- with(bbri_backtest, ave(stock_buy, rleid(stock_buy), FUN = cumsum))
```
...
```{r}
bbri_cv <- bbri_analisa2[71:896,] %>%
  mutate_if(is.character, as.factor)

bbri_cv2 <- bbri_analisa2[-(1:70),]
```
...
```{r}
bbri_test$pred_dt <- pred_model_dt

bbri_test
```

```{r}
bbri_dt_backtest <- bbri_test %>% 
  filter(pred_dt != "Hold") %>% 
...
```{r}
bbri_test$pred_rf <- rm_pred

bbri_test
```

```{r}
bbri_rf_backtest <- bbri_test %>% 
  filter(pred_rf != "Hold") %>% 
